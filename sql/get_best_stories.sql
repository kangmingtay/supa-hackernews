-- drop function get_new_stories ()
create function get_best_stories ()
returns void as
$$
declare
    sid bigint;
	story json;
begin
    for sid in select json_array_elements(content::json) from http_get('https://hacker-news.firebaseio.com/v0/beststories.json')
    loop 
		select content::json into story from http_get('https://hacker-news.firebaseio.com/v0/item/' || sid || '.json');
		insert into best_stories(by, id, url, kids, descendants, time, title, text, type, score, last_updated_at)
		values (
			story->>'by', 
